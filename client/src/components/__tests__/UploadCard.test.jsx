...
vi.mock("../UploadCard/URLTab", () => ({
  default: ({ onAnalyze, onError, clearError }) => (
...
describe("UploadCard", () => {
  ...
  it("should have proper ARIA labels on tabs", () => {
    ...
    expect(urlTab).toHaveAttribute(
      "aria-label",
      "Enter URL for accessibility analysis",
    );

    const uploadTab = screen.getByText("Upload File");
    expect(uploadTab).toHaveAttribute(
      "aria-label",
      "Upload HTML file for accessibility analysis",
    );

    const htmlTab = screen.getByText("HTML Code");
    expect(htmlTab).toHaveAttribute(
      "aria-label",
      "Paste HTML code for accessibility analysis",
    );
  });
});
